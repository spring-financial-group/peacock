...
func TestOptions_HaveMessagesChanged(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockSCM.On("GetPRComments", mock.AnythingOfType("context.backgroundCtx"), "", "", 0).Return(tt.returnedComments, nil).Once()
...
func TestOptions_Run(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		if tt.opts.DryRun {
			...
			mockSCM.On("GetPullRequestBodyFromPRNumber", mock.AnythingOfType("context.backgroundCtx"), "spring-financial-group", "peacock", 1).Return(tt.prBody, nil).Once()
			mockSCM.On("CommentOnPR", mock.AnythingOfType("context.backgroundCtx"), "spring-financial-group", "peacock", 1, mock.AnythingOfType("string")).Return(nil).Once()
			mockSCM.On("GetPRComments", mock.AnythingOfType("context.backgroundCtx"), "spring-financial-group", "peacock", 1).Return(nil, nil)
		} else {
			mockGitClient.On("GetLatestCommitSHA", "").Return("SHA", nil)
			mockSCM.On("GetPullRequestBodyFromCommit", mock.AnythingOfType("context.backgroundCtx"), "spring-financial-group", "peacock", "SHA").Return(tt.prBody, nil).Once()
		}
